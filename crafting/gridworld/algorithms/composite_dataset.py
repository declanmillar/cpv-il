...
class ActionToTensor(object):
    ...
    def convert_image(self, image):
        if isinstance(image, torch.Tensor):
            ...
            return image.type(torch.FloatTensor) / 255.0
        else:
            image[-1, -1, -1] = 0
            image = image.transpose((2, 0, 1))
            return torch.from_numpy(image).type(torch.FloatTensor) / 255.0
...
class CompositeDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        directory="/persistent/affordance_world/data/paired_compositions2/",
        train=True,
        size=None,
        include_ref=True,
        pickup_balance=0,
    ...
    def __getitem__(self, idx):
        ...
        if len(exp) < 4:
            ...
            while len(exp) < 3:
                exp = [exp[0]] + exp
        index = np.random.randint(0, len(exp) - 2)
        sample = {"image": exp[index], "next_image": exp[index + 1]}
        sample = self.transform(sample)

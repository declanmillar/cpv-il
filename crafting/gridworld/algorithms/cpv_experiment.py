...
parser.add_argument(
    "--batch-size", type=int, default=256, metavar="N", help="input batch size for training (default: 128)"
)
...
parser.add_argument("--train_size", type=int, default=None, help="which task to run")
parser.add_argument("--test_size", type=int, default=1000, help="which task to run")
parser.add_argument("--resume", type=str, default=None, help="A model to load")
parser.add_argument("--modelV", type=int, default=3, help="A model to load")
parser.add_argument("--feature-dim", type=int, default=256)
parser.add_argument("-H", action="store_true", default=False, help="use hom loss")
parser.add_argument("-P", action="store_true", default=False, help="use pair loss")
parser.add_argument("--letter", type=str, default="A", help="Suffix to model weights")
...
train_loader = torch.utils.data.DataLoader(
    CompositeDataset(directory=data_dir, train=True, size=args.train_size),
...
test_loader = torch.utils.data.DataLoader(
    CompositeDataset(directory=data_dir, train=False, size=args.test_size),
    batch_size=args.batch_size,
    shuffle=True,
    worker_init_fn=worker_init_fn,
    **kwargs
...
def train(epoch):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        logpi, goal_feat, final_feat, back_feat = model.forward(
            first_image, image, pre_image, post_image, final_image=final_image
        )
    ...
    print(
        "====> Epoch: {} Average loss: {:.4f} Accuracy: {:.4f} FeatLoss: {:.4f} ".format(
...
def test(epoch):
    ...
    print(
        "====> Test set loss: {:.4f} Accuracy: {:.4f} FeatLoss: {:.4f} ".format(
            test_loss / num_batch, test_acc / num_batch, total_feat_loss / num_batch
        )
    )

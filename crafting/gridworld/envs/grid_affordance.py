...
CLIP = False
if CLIP:
    base_sprite = np.zeros((3, 3, 3)) / 10.0
else:
    base_sprite = np.ones((3, 3, 3)) / 10.0
sprites = []
for i in range(3):
    for j in range(3):
...
# BGR
SPRITES["agent"][0, 0] = np.array([0 / 255.0, 0 / 255.0, 255 / 255.0])
SPRITES["rock"][0, 1] = np.array([211 / 255.0, 211 / 255.0, 211 / 255.0])
SPRITES["hammer"][0, 2] = np.array([204 / 255.0, 204 / 255.0, 0 / 255.0])
SPRITES["tree"][1, 0] = np.array([34 / 255.0, 133 / 255.0, 34 / 255.0])
SPRITES["bread"][1, 1] = np.array([0 / 255.0, 215 / 255.0, 255 / 255.0])
SPRITES["wheat"][1, 2] = np.array([10 / 255.0, 215 / 255.0, 100 / 255.0])
SPRITES["sticks"][2, 0] = np.array([45 / 255.0, 82 / 255.0, 160 / 255.0])
SPRITES["axe"][2, 1] = np.array([255 / 255.0, 102 / 255.0, 102 / 255.0])
SPRITES["house"][2, 2] = np.array([153 / 255.0, 52 / 255.0, 255 / 255.0])
    ...
    def __init__(
        ...
        if self.state_obs:
            ...
            self.observation_space = spaces.Box(
                low=0, high=1.0, shape=((self.nrow + 1) * res * 2 * self.ncol * res * 2 * 3 + goal_dim,)
            )

        else:
            self.observation_space = spaces.Box(
                low=0, high=1.0, shape=((self.nrow + 1) * res * self.ncol * res * 3 + goal_dim,)
            )
        self.objects = []
        self.res = res
        ...
        if pretty_renderable:
            ...
            self.pretty_render_sprites = {
                asset.split("/")[-1].split(".")[0]: cv2.imread(asset) for asset in asset_paths
            }
            ...
            for obj in to_get_obs:
                ...
                if root is not None:
                    if root in self.SPRITES:
                        ...
                        img[
                            row * self.res : (row + 1) * self.res, col * self.res : (col + 1) * self.res, :
                        ] += self.SPRITES[root]

            if self.visible_agent:
                row, col = self.state[AGENT]
                img[row * self.res : (row + 1) * self.res, col * self.res : (col + 1) * self.res, :] += self.SPRITES[
                    AGENT
                ]
            ...
            for obj in to_get_obs:
                ...
                if root is not None:
                    if root in self.SPRITES:
                        ...
                        img[
                            row * self.renderres : (row + 1) * self.renderres,
                            col * self.renderres : (col + 1) * self.renderres,
                            :,
                        ] += self.BIGSPRITES[root]
        ...
        if mode == "rgb":
            ...
            for row in range(self.nrow):
                for col in range(self.ncol):
                    img[
                        row * self.pretty_render_res : (row + 1) * self.pretty_render_res,
                        col * self.pretty_render_res : (col + 1) * self.pretty_render_res,
                    ] = grass
            to_get_obs = self.state["object_positions"].keys()
            ...
            if len(self.state[HOLDING]) > 0:
                ...
                img[
                    w - self.pretty_render_res : w, self.pretty_render_res : self.pretty_render_res * 2

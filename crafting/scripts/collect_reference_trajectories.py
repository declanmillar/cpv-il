...
def main():
    ...
    for n in [1, 2, 4, 8, 16]:
        base_directory = "data/Oct_references_len" + str(n) + "/"
            ...
            if True:
                ...
                if data["ref"][0]:
                    for style in ["ref"]:
                        ...
                        saved += 1
                        np.save(
                            directory + "/" + policy_n + "_episode{:04d}_".format(episode) + style + ".npy", image_arr
                        )
                        object_counts = {obj: [s["object_counts"][obj] for s in states] for obj in OBJECTS}
                        with open(
                            directory + "/" + policy_n + "_episode{:04d}_".format(episode) + style + "_counts.pkl", "wb"
                        ) as f:

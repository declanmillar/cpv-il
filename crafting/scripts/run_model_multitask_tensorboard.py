...
from gridworld.policies.composite_policy import CompositePolicy, pol_index
...
parser.add_argument("--tasks", type=str, nargs="*", default=None, help="which task to run")
parser.add_argument("--model", type=str, help="exp name")
parser.add_argument("--tb", type=str, default=None, help="tensorboard dir")
parser.add_argument("-N", type=int, default=1, help="number of rollouts")
parser.add_argument("--render", type=str, default=False, help="task name")
parser.add_argument("--type", type=str, choices=["V3", "TEC", "Naive", "TE", "Duan"])
parser.add_argument("--start-idx", type=int, default=0, help="model index to start at")
parser.add_argument("--end-idx", type=int, default=None, help="model index to end at")
parser.add_argument("--num-policies", type=int, default=4, help="length of trajecotries to eval on")
parser.add_argument("--cpu", action="store_true", default=False, help="use cpu")
parser.add_argument("--num-sum", type=int, default=1, help="sumembeddings")
parser.add_argument("--feature-dim", type=int, default=256)
parser.add_argument("--avoidant", action="store_true", default=False, help="use avoidant dataset")
parser.add_argument("--relu", action="store_true", default=False, help="relu the representation")
args = parser.parse_args()
RENDER = args.render
print(args)
policytype, modeltype = {
    "V3": (LearnedBCPolicy, CompositeDotModelV3),
...
def run_model(policy, path, model_epoch, num_pol, num_sum):
    if args.type in EMBEDDING_MODELS:
        ...
        if isinstance(path, list):
            policy_list, policy_n, ref_counts_diff, first_img = process_path_list(
                policy, path, num_pol, num_sum
            )  # first_img is actually list of all images
        else:
            policy_list, policy_n, ref_counts_diff, first_img = process_path(
                policy, path, num_pol, num_sum
            )  # first_img is actually list of all images
    else:
        if isinstance(path, list):
            policy_list, policy_n, ref_counts_diff, first_img, last_img = process_path_list(
                policy, path, num_pol, num_sum
            )
        else:
            policy_list, policy_n, ref_counts_diff, first_img, last_img = process_path(policy, path, num_pol, num_sum)
    composite_policy = CompositePolicy(policy_list, H.action_space, H, noise_level=0.0)
    task_success = []
    num_steps = []
    diffs = []
        ...
        if epoch > max_epoch:
            ...
            if epoch >= args.start_idx and epoch % 40 == 0:
                ...
                policy = policytype(
                    ...
                    model=modeltype,
                    agent_centric=False,
                    env_dim=(H.res * (H.nrow + 1), H.res * (H.ncol), 3),
                    task_embedding_dim=args.feature_dim,
                    relu=args.relu,
                )
                for num_pol, num_sum in [(1, 1), (1, 2), (2, 1), (2, 2), (2, 4), (4, 1), (4, 2), (8, 1), (16, 1)]:
                    ...
                    if len(num_steps) > 0:
                        ...
                        avg_length = 0
                    writer.add_scalar(
                        "onpolicy/{}pol_{}sum".format(num_pol, num_sum), sum(successes) / len(successes), epoch
                    )
                    print(
                        "epoch",
                        epoch,
